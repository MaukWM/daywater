...
def build_tools(
    ...
    *,
    ...
    if Capability.STATIC_RE in spec.capabilities:
        from src.agent.tools.ghidra import (
            add_note,
            callees,
            callers,
            decompile,
    ...
    if Capability.DISCOVERY in spec.capabilities:
        from src.agent.tools.discovery import (
            analyze_binary,
            extract_iso,
            list_iso_contents,
            switch_binary,
    ...
    if Capability.FRAME_CAPTURE in spec.capabilities and session is not None:
        from src.agent.tools.capture import capture_screenshot
    ...
    if Capability.INPUT_INJECTION in spec.capabilities:
        ...
        from src.agent.tools.controller import (
            press_button,
            sample_position,
            set_stick,
            wait,
    ...
    if Capability.GECKO_INJECTION in spec.capabilities:
        from src.agent.tools.ppc import assemble_ppc, make_c2_hook

        tools += [assemble_ppc(), make_c2_hook()]

    # Knowledge base (always)
    from src.agent.tools.findings import list_findings, save_finding
    from src.agent.tools.research import list_research, read_research, write_research
    ...
    if savestate_root is not None:
        from src.agent.tools.findings import (
    ...
    from src.agent.tools.winddown import wrap_tools_with_winddown
